...
void dsAppClient::on_luaerror( const dsstring& p_errstr )
{
    _asm nop
}
...
bool dsAppClient::OnIdleAppInit( void )
{
    ...
    m_luacontext = _DRAWSPACE_NEW_( LuaContext, LuaContext );


    m_luaerrorhandler = _DRAWSPACE_NEW_( LuaErrorHandler, LuaErrorHandler( this, &dsAppClient::on_luaerror ) );
    m_luacontext->RegisterErrorHandler( m_luaerrorhandler );

    m_luacontext->Startup();


    m_luacontext->Exec( "fail=8 function mob_behaviour() end" );
